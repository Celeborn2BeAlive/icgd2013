...
A face $f \in X$ is \myem{a facet of $X$} if $f$ is not a proper face of any face of $X$. We denote by $\princ{X}$ the set composed of all facets of $X$. The \myem{dimension of $X$} is $\mydim{X} = \max\{\mydim{f} \suchthat f \in X\}$. If $\mydim{X} = d$, then we say that $X$ is a $d$-complex.
...
In the following, we give a new method requiring no user input, for obtaining a curvilinear skeleton yielding satisfactory visual properties. This will be achieved by adding, in the set $W$ of Alg.~\ref{algo::pardircollapse_v0}, some faces from the original object during the thinning process.

\subsection{The lifespan of a face}
In the following, we define additional functions in the cubical complex, which are essential for the filtering step of the skeleton. The first one we present is the \myem{death date} of a face.
...
In order to extend all these previous maps defined in $\Zset^n$ to the cubical complex framework, let us introduce the map $\V2C^{-1}$, inverse of the bijective map $\V2C : \Zset^{n} \rightarrow \subfaces{n}{n}$ defined in Sec.~\ref{sec::binary_to_cubic}. It is used to project any $n$-face of $\allfaces{n}$ into $\Zset^n$. We indifferently use $\V2C^{-1}$ as a map from $\subfaces{n}{n}$ to $\Zset^n$, and as a map from $\mathcal{P}(\subfaces{n}{n})$ to $\mathcal{P}(\Zset^n)$.
