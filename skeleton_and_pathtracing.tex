...
\label{sec::intro}
Path tracing algorithms are able to render photorealistic images from a scene description by simulating light propagation. The photorealistic aspect is achieved by simulating diffusion of light: elements of the scene which are illuminated by a light source can cast part of the light they receive. Thanks to this indirect lighting, parts of the scene which are not directly illuminated by any light source actually receive light, thus giving a nice effect of light diffusion and soft shadows to the image. 

To do so, a path tracing algorithm sends rays (of light) with a given energy from the camera, and make them bounce around the scene until they encounter a light source. When a ray hit a point of an object, it loses some energy and gathers some information on the object's material. Then, the ray is reemitted in a direction randomly chosen among various possibilities. Once a ray reaches a light source, the information it gathered through its journey in the scene is averaged in order to decide of the colour of the first impact point. This color is then allocated to one of the pixel of the image, and the process is repeated for each pixel of the output image. In order to obtain a realistic result, it is necessary to integrate various bouncing directions for the rays: this is done by sending many rays for a same pixel: the more rays, the longer the algorithm will take to produce an image, but the more realistic (close to the results obtained by reemitting rays in all possible directions) the result will be. A path tracing can be seen as a random walk process.

The bouncing direction of a ray depends on the texture of the hit point: to each texture is associated a probability distribution function which gives, for a given incoming direction, the probability of reemitting the ray in another directions. The bouncing direction is randomly chosen among the possible values of the probability distribution function. Choosing only one bouncing direction produces a sampling effect which creates noise in the resulting image (due to points where unusual directions were chosen): this noise is naturally reduced by shooting more rays per each image pixel, but unless one shoots a big amount of rays per pixels (and waits a long time before obtaining an image), the noise will always be present in the output image. 

In order to avoid infinite bouncing of rays in the scene, a lower limit is set for each ray's energy: below this threshold, a ray will be considered to bring a too small contribution to the pixel's color and will be eliminated from the scene. However, a lost ray is as a waste of computation time, and if one wants to speed up the path tracing algorithm, he should try to minimize the amount of lost rays.

%Precomputation time must remain fast > skeleton !!
...
The \myem{closure} of a set of faces $X$ is the set $\closure{X}=\union\{\cell{f} \vert f \in X\}$. The set $\Compl{X}$ is $\allfaces{n} \setminus X$. 
...
Intuitively, we want our thinning algorithm to remove free faces of a complex \quotes{layer by layer} and avoid having unequal thinning of the input complex. Therefore, we want each execution of the algorithm to remove free faces located on the border of the input complex. 
...
As previously said, our goal is to use the skeleton of a scene in order to guide light rays from the camera of the scene towards the light. In order to achieve this objective, we need a skeleton which \quotes{looks like} the original scene it was calculated from. 
...
In the following, we define additional functions in the cubical complex, related to the thinning process (Alg.~\ref{algo::pardircollapse_v0}), which are essential for the filtering step of the skeleton. The first one we present is the \myem{death date} of a face.
...
The lifespan of a face $f$ of $X$ indicates how many "rounds" this face "survives" as a facet in $X$, when removing free faces with Alg.~\ref{algo::pardircollapse_v0}, and is a good indicator of how important a face can be in an object. Typically, the higher the lifespan is, and the more representative of an object's visual feature the face is. The lifespan, sometimes called \myem{saliency}, was used in \cite{Liu2009} (with the name "medial persistence") in order to propose a thinning algorithm in cubical complexes based on two parameters.
