\documentclass[runninghead,envcountsame]{llncs}
...
\caption{\label{fig::intro_image} Images $a$ and $b$ shows some rays of the path tracing guided by the material of the objects (image $a$) or our skeleton based method (image $b$). Images $c$ and $d$ point out the noise reduction between the classic path tracing (image $c$) and our method (image $d$).}
\end{center}
\end{figure}
...
Then, we expose a new method, based on such skeletons, to enhance the results and the performance of the path tracing algorithm (see Sec.~\ref{sec::appli_pathtrace}).
